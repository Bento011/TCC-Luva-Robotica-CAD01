...
const int fato_motor[numSensors] = {-1, 1, -1, 1, 1};
...
bool flag_erro[numSensors] = {false, false, false, false, false};
bool flag_OSC = false;
  ...
  if (botoes == 0) {
    ...
    // Define ponto Médio do paciente
      ...
      for (servoIndex = 0; servoIndex < numSensors; servoIndex++) {
        ...
        Serial.print("MedBend: " + String(SafeBend[servoIndex]) + " " + (MedBend[servoIndex]) + " ; ");
      }
      flag_max = 0;
      flag_min = 0;
      botoes = 1;
...
void mexeMotor() {
  ...
  Serial.println(iter);
  int passo[numSensors] = { 0, 0, 0, 0, 0 };
  for (servoIndex = 0; servoIndex < numSensors; servoIndex++) {
    int from_flex = ValorFlex(servoIndex);
    int to_flex = setPoint_flex[servoIndex];
    ...
    if (abs(erro[servoIndex]) <= setPoint_flex[servoIndex]/5) {
      ...
      passo[servoIndex] = 1 * fato_motor[servoIndex];
